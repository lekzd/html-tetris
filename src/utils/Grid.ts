...
export class Grid<T> {
  ...
  set(x: number, y: number, data: T): Grid<T> {
  ...
  forEach(callback: iteratorCallback<T, void>): Grid<T> {
    this.data.forEach(this.iteratee.bind(this, callback));

    return this;
  }

  map(callback: iteratorCallback<T, void>): any[] {
    return this.data.map(this.iteratee.bind(this, callback));
  }

  filter(filterFunction: iteratorCallback<T, boolean>): Grid<T> {
    let result = new Grid<T>(this.size);
